...
--randomly filtering 8 unique customer ids using Joins concept.
SELECT
  s.customer_id,
  f.plan_id, 
  f.plan_name,  
...
WHERE 
  s.customer_id IN (76,54,81,298,495,376,118,19) -- selected 8 customers
...
SELECT plan_name,
       count(customer_id) customer_count,
       round(100 *count(DISTINCT customer_id) / (SELECT count(DISTINCT customer_id) FROM subscriptions), 2) AS 'customer percentage'                                                                'customer percentage'
FROM subscriptions
JOIN plans USING (plan_id)
...
SELECT plan_id,
       ...
       round(100*count(customer_id) /
               (SELECT COUNT(DISTINCT customer_id)
                FROM subscriptions), 3) AS percentage_breakdown
...
WITH trial_plan_customer_cte AS
  ...
  (SELECT *
   ...
   JOIN plans USING (plan_id)
   WHERE plan_id=3)
SELECT round(avg(datediff(annual_plan_customer_cte.start_date, trial_plan_customer_cte.start_date)), 2)AS average_conversion_days
